...
public class PrototypeController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		thirdpartservices_service = ServiceManager.createThirdPartServices();
	...
	*/
	public void genereateOpInvariantPanel() {
		...
		//entities invariants
		entities = ATMSystemImpl.opINVRelatedEntity.get("withDraw");
		if (entities != null) {
			for (String opRelatedEntities : entities) {				
				for (Entry<String, String> inv : entity_invariants_map.entrySet()) {
			...
			Label l = new Label("withDraw" + " is no related invariants");
			l.setPadding(new Insets(8, 8, 8, 8));
			v.getChildren().add(l);
		}
		...
		opInvariantPanel.put("withDraw", v);
		
		v = new VBox();
		
		//entities invariants
		entities = ATMSystemImpl.opINVRelatedEntity.get("deposit");
		if (entities != null) {
			for (String opRelatedEntities : entities) {				
				for (Entry<String, String> inv : entity_invariants_map.entrySet()) {
			...
			Label l = new Label("deposit" + " is no related invariants");
			l.setPadding(new Insets(8, 8, 8, 8));
			v.getChildren().add(l);
		}
		...
		opInvariantPanel.put("deposit", v);
	...
	*/
	...
	public void generateObjectTable() {
		...
		TableColumn<Map<String, String>, String> tableUser_Phone = new TableColumn<Map<String, String>, String>("Phone");
		tableUser_Phone.setMinWidth("Phone".length()*10);
		tableUser_Phone.setCellValueFactory(new Callback<CellDataFeatures<Map<String, String>, String>, ObservableValue<String>>() {	   
			@Override
		    public ObservableValue<String> call(CellDataFeatures<Map<String, String>, String> data) {
		        return new ReadOnlyStringWrapper(data.getValue().get("Phone"));
		    }
		});	
		tableUser.getColumns().add(tableUser_Phone);
		...
		TableView<Map<String, String>> tableTranscation = new TableView<Map<String, String>>();

		//super entity attribute column
						
		//attributes table column
		TableColumn<Map<String, String>, String> tableTranscation_WithdrawedNum = new TableColumn<Map<String, String>, String>("WithdrawedNum");
		tableTranscation_WithdrawedNum.setMinWidth("WithdrawedNum".length()*10);
		tableTranscation_WithdrawedNum.setCellValueFactory(new Callback<CellDataFeatures<Map<String, String>, String>, ObservableValue<String>>() {	   
			@Override
		    public ObservableValue<String> call(CellDataFeatures<Map<String, String>, String> data) {
		        return new ReadOnlyStringWrapper(data.getValue().get("WithdrawedNum"));
		    }
		});	
		tableTranscation.getColumns().add(tableTranscation_WithdrawedNum);
		TableColumn<Map<String, String>, String> tableTranscation_BalanceAfterWithdraw = new TableColumn<Map<String, String>, String>("BalanceAfterWithdraw");
		tableTranscation_BalanceAfterWithdraw.setMinWidth("BalanceAfterWithdraw".length()*10);
		tableTranscation_BalanceAfterWithdraw.setCellValueFactory(new Callback<CellDataFeatures<Map<String, String>, String>, ObservableValue<String>>() {	   
			@Override
		    public ObservableValue<String> call(CellDataFeatures<Map<String, String>, String> data) {
		        return new ReadOnlyStringWrapper(data.getValue().get("BalanceAfterWithdraw"));
		    }
		});	
		tableTranscation.getColumns().add(tableTranscation_BalanceAfterWithdraw);
		
		//table data
		ObservableList<Map<String, String>> dataTranscation = FXCollections.observableArrayList();
		List<Transcation> rsTranscation = EntityManager.getAllInstancesOf("Transcation");
		for (Transcation r : rsTranscation) {
			//table entry
			Map<String, String> unit = new HashMap<String, String>();
			
			unit.put("WithdrawedNum", String.valueOf(r.getWithdrawedNum()));
			unit.put("BalanceAfterWithdraw", String.valueOf(r.getBalanceAfterWithdraw()));

			dataTranscation.add(unit);
		}
		
		tableTranscation.getSelectionModel().selectedIndexProperty().addListener(
							 (observable, oldValue, newValue) ->  { 
							 										 //get selected index
							 										 objectindex = tableTranscation.getSelectionModel().getSelectedIndex();
							 			 				 			 System.out.println("select: " + objectindex);

							 			 				 			 //update association object information
							 			 				 			 if (objectindex != -1)
										 			 					 updateAssociation("Transcation", objectindex);
							 			 				 			 
							 			 				 		  });
		
		tableTranscation.setItems(dataTranscation);
		allObjectTables.put("Transcation", tableTranscation);
	...
	*/ 
	...
	public void updateTranscationTable(List<Transcation> rsTranscation) {
			ObservableList<Map<String, String>> dataTranscation = FXCollections.observableArrayList();
			for (Transcation r : rsTranscation) {
				Map<String, String> unit = new HashMap<String, String>();
			...
			allObjectTables.get("Transcation").setItems(dataTranscation);
	}
	...
	*/ 
	...
	public void updateTranscationTable() {
			ObservableList<Map<String, String>> dataTranscation = FXCollections.observableArrayList();
			List<Transcation> rsTranscation = EntityManager.getAllInstancesOf("Transcation");
			for (Transcation r : rsTranscation) {
				Map<String, String> unit = new HashMap<String, String>();
	...
	public void classStatisicBingding() {
		 ...
		 classInfodata = FXCollections.observableArrayList();
	 	 ...
	 	 transcation = new ClassInfo("Transcation", EntityManager.getAllInstancesOf("Transcation").size());
	 	 classInfodata.add(transcation);
	...
	public void classStatisticUpdate() {
	 	 ...
	 	 transcation.setNumber(EntityManager.getAllInstancesOf("Transcation").size());
	...
	public void associationStatisicBingding() {
		...
		AssociationInfo BankCard_associatition_Has = new AssociationInfo("BankCard", "Transcation", "Has", true);
		...
		AssociationInfo User_associatition_HasTrans = new AssociationInfo("User", "Transcation", "HasTrans", true);
		User_association_data.add(User_associatition_HasTrans);
		
		allassociationData.put("User", User_association_data);
		
		ObservableList<AssociationInfo> Transcation_association_data = FXCollections.observableArrayList();
		AssociationInfo Transcation_associatition_InvolvedCard = new AssociationInfo("Transcation", "BankCard", "InvolvedCard", false);
		Transcation_association_data.add(Transcation_associatition_InvolvedCard);
		
		allassociationData.put("Transcation", Transcation_association_data);
	...
	public void prepareData() {
		...
		definitions_map.put("inputCard", "bc:BankCard = BankCard.allInstance()->any(c:BankCard | c.CardID = cardid)\n");
		definitions_map.put("createBankCard", "bankcard:BankCard = BankCard.allInstance()->any(ban:BankCard | ban.CardID = cardid)\n");
		definitions_map.put("queryBankCard", "bankcard:BankCard = BankCard.allInstance()->any(ban:BankCard | ban.CardID = cardid)\n");
		definitions_map.put("modifyBankCard", "bankcard:BankCard = BankCard.allInstance()->any(ban:BankCard | ban.CardID = cardid)\n");
		definitions_map.put("deleteBankCard", "bankcard:BankCard = BankCard.allInstance()->any(ban:BankCard | ban.CardID = cardid)\n");
		...
		preconditions_map.put("withDraw", "self.PasswordValidated = true and\nself.CardIDValidated = true and\nself.InputCard.oclIsUndefined() = false and\nself.InputCard.Balance >= quantity\n");
		preconditions_map.put("deposit", "self.PasswordValidated = true and\nself.CardIDValidated = true and\nself.InputCard.oclIsUndefined() = false and\nquantity >= 0\n");
		...
		entity_invariants_map.put("Transcation_WithdrawedNumGreatAndEqualZero"," WithdrawedNum >= 0");
		entity_invariants_map.put("Transcation_BalanceAfterWithdrawGreatAndEqualZero"," BalanceAfterWithdraw >= 0");
	...
	public void generatOperationPane() {
		 ...
		 withDraw.setHgap(4);
		 withDraw.setVgap(6);
		 withDraw.setPadding(new Insets(8, 8, 8, 8));
		 
		 ObservableList<Node> withDraw_content = withDraw.getChildren();
		 Label withDraw_quantity_label = new Label("quantity:");
		 withDraw_quantity_label.setMinWidth(Region.USE_PREF_SIZE);
		 withDraw_content.add(withDraw_quantity_label);
		 GridPane.setConstraints(withDraw_quantity_label, 0, 0);
		 
		 withDraw_quantity_t = new TextField();
		 withDraw_content.add(withDraw_quantity_t);
		 withDraw_quantity_t.setMinWidth(Region.USE_PREF_SIZE);
		 GridPane.setConstraints(withDraw_quantity_t, 1, 0);
		 operationPanels.put("withDraw", withDraw);
		 
		 // ==================== GridPane_deposit ====================
		 GridPane deposit = new GridPane();
		 deposit.setHgap(4);
		 deposit.setVgap(6);
		 deposit.setPadding(new Insets(8, 8, 8, 8));
		 
		 ObservableList<Node> deposit_content = deposit.getChildren();
		 Label deposit_quantity_label = new Label("quantity:");
		 deposit_quantity_label.setMinWidth(Region.USE_PREF_SIZE);
		 deposit_content.add(deposit_quantity_label);
		 GridPane.setConstraints(deposit_quantity_label, 0, 0);
		 
		 deposit_quantity_t = new TextField();
		 deposit_content.add(deposit_quantity_t);
		 deposit_quantity_t.setMinWidth(Region.USE_PREF_SIZE);
		 GridPane.setConstraints(deposit_quantity_t, 1, 0);
		 operationPanels.put("deposit", deposit);
	...
	public void actorTreeViewBinding() {
		...
		TreeItem<String> treeRootpeople = new TreeItem<String>("Root node");
			TreeItem<String> subTreeRoot_Register = new TreeItem<String>("Register");
			subTreeRoot_Register.getChildren().addAll(Arrays.asList(
				 		new TreeItem<String>("deposit"),
				 		new TreeItem<String>("inputPassword"),
				 		new TreeItem<String>("inputCard"),
				 		new TreeItem<String>("ejectCard"),
				 		new TreeItem<String>("withDraw"),
				 		new TreeItem<String>("transfer"),
				 		new TreeItem<String>("printReceipt"),
				 		new TreeItem<String>("checkBalance")
				 		));	
		
		treeRootpeople.getChildren().addAll(Arrays.asList(
			subTreeRoot_Register
			));
		
		treeRootpeople.setExpanded(true);

		actor_treeview_people.setShowRoot(false);
		actor_treeview_people.setRoot(treeRootpeople);
		
		actor_treeview_people.getSelectionModel().selectedItemProperty().addListener(
						 (observable, oldValue, newValue) -> { 
	...
	*/			
	...
	public void withDraw() {
		...
		System.out.println("execute withDraw");
		String time = String.format("%1$tH:%1$tM:%1$tS", System.currentTimeMillis());
		log.appendText(time + " -- execute operation: withDraw in service: ATMSystem ");
		
		try {
			//invoke op with parameters
			//return value is primitive type, bind result to label.
			String result = String.valueOf(atmsystem_service.withDraw(
			Integer.valueOf(withDraw_quantity_t.getText())
			));	
			Label l = new Label(result);
			l.setPadding(new Insets(8, 8, 8, 8));
			operation_return_pane.setContent(l);
	...
	public void deposit() {
		...
		System.out.println("execute deposit");
		String time = String.format("%1$tH:%1$tM:%1$tS", System.currentTimeMillis());
		log.appendText(time + " -- execute operation: deposit in service: ATMSystem ");
		
		try {
			//invoke op with parameters
			//return value is primitive type, bind result to label.
			String result = String.valueOf(atmsystem_service.deposit(
			Float.valueOf(deposit_quantity_t.getText())
			));	
			Label l = new Label(result);
			l.setPadding(new Insets(8, 8, 8, 8));
			operation_return_pane.setContent(l);
	...
	@FXML
	TreeView<String> actor_treeview_people;
	@FXML
	...
	ThirdPartServices thirdpartservices_service;
	
	ClassInfo bankcard;
	ClassInfo user;
	ClassInfo transcation;
	...
	TextField withDraw_quantity_t;
	TextField deposit_quantity_t;
